    ...
    <Page variant="split">
      <Section variant="image">
        ...
        <Section variant="tagline">
          <p className="mb-4 text-4xl font-bold leading-tight drop-shadow-lg text-foreground">Empower your enterprise with intelligent workflows.</p>
          <p className="text-lg text-muted-foreground drop-shadow">The next generation ERP system designed for scale, speed, and seamless integration.</p>
        </Section>
      </Section>
      ...
      <Section variant="form">
        ...
        <Card variant="login">
          ...
          <CardContent>
            <form id="login-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <FormField label="Password" variant="erp" icon={<Lock className="h-5 w-5" />} type={showPassword ? 'text' : 'password'} placeholder="••••••••" className="pr-12" labelRight={<a href="#" className="text-sm font-medium text-primary hover:text-primary/80">Forgot password?</a>}{...register('password', { required: true })}>
        ...
        <p className="absolute bottom-8 right-8 text-xs text-muted-foreground">© 2026 Interprise Inc.</p>
      </Section>
    </Page>
  );
}
